	...
	public static void regist() throws IOException, ClassNotFoundException {
		...
		outer: while (true) {
			...
			try {
				...
				ArrayList<User> list = (ArrayList<User>) in.readObject(); // 这个list中保存所有User
				for (User user : list) {
				...
				list.add(newUser); // 如果遍历结束, 没有相同的, 则将新User存进list, 将list写入文件
				ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("user.txt"));
				out.writeObject(list);
				out.close();
				System.out.println("注册成功");
				break outer;
			} catch (EOFException e) { // 如果抛出EOFException异常, 那么说明文件中没有List, 写出一个.
				ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("user.txt"));
